...
dependencies='authconfig compat-openldap oddjob-mkhomedir openldap-clients openssh-clients openssh-server sssd sssd-client ksh pam_radius'
...
az storage blob download --account-name $centralized_storage_account --container-name common --name azure_bastion_config_files_updated.zip --file azure_bastion_config_files_updated.zip --auth-mode login
sleep 10

# extract config files from downloaded zip & move into unzipped folder
unzip -n /tmp/2fa/azure_bastion_config_files_updated.zip
sleep 10
cd /tmp/2fa/azure_bastion_config_files_updated

# remove zip file we downloaded - we no longer need it
rm -rf /tmp/2fa/azure_bastion_config_files_updated.zip
...
for line in `ls /tmp/2fa/azure_bastion_config_files_updated`; do

...
locals {
  windows_base = {
    exists = {
      1 = {
        asg_name   = "asg-${var.subcode}-${var.asg_uai}-windows-base",
  ...
  asgs = merge(
    local.windows_base[var.default_asgs.windows_base ? "exists" : "not_exists"],
...
#Create Default Windows NIC ASG Associations

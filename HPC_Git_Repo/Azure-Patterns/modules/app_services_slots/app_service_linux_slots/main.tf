...
resource "azurerm_monitor_diagnostic_setting" "diagnos" {
  for_each                   = toset(var.staging_slot_name)
  name                       = "azapp-lx-${var.uai}-${var.appName}-${var.purpose}-${each.value}-ds"
  target_resource_id         = azurerm_linux_web_app_slot.linux_webapp_slot[each.key].id
...
resource "azurerm_private_endpoint" "app_service_pvtendpoint_slot" {
  for_each            = toset(var.staging_slot_name)
  name                = "pe-azapp-lx-${var.uai}-${var.appName}-${var.purpose}-${each.value}"
  location            = var.region
  resource_group_name = data.azurerm_resource_group.rg.name
  ...
  private_service_connection {
    name                           = "pes-azapp-lx-${var.uai}-${var.appName}-${var.purpose}-${each.value}"
